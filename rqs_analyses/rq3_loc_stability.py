...
OUT_DIR = PROJECT_ROOT / "rq3_localization_stability"
OUT_DIR.mkdir(parents=True, exist_ok=True)

OUT_SUMMARY_CSV = OUT_DIR / "localization_stability_fixed_threshold_summary_nsga.csv"
OUT_PER_SAMPLE_CSV = OUT_DIR / "localization_stability_fixed_threshold_per_sample_nsga.csv"

ORIG_IOU_COL = "orig_avg_iou_same"
PERT_IOU_COL = "pert_avg_iou_same"
...
def load_model_rows(model_name: str, folder: Path, csv_names):
    ...
    for csv_name in csv_names:
        ...
        validate_columns(
            df,
            [ORIG_IOU_COL, PERT_IOU_COL, FAILURE_FLAG_COL],
            f"{model_name} | {csv_name}"
        )

        keep_cols = [ORIG_IOU_COL, PERT_IOU_COL, FAILURE_FLAG_COL]
        ...
        df[ORIG_IOU_COL] = pd.to_numeric(df[ORIG_IOU_COL], errors="coerce")
        df[PERT_IOU_COL] = pd.to_numeric(df[PERT_IOU_COL], errors="coerce")
        df[FAILURE_FLAG_COL] = pd.to_numeric(df[FAILURE_FLAG_COL], errors="coerce")

        df = df.dropna(subset=[ORIG_IOU_COL, PERT_IOU_COL, FAILURE_FLAG_COL]).copy()
        frames.append(df)
...
def compute_non_failing_deviations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    non_fail_df["iou_deviation_abs"] = (
        non_fail_df[ORIG_IOU_COL] - non_fail_df[PERT_IOU_COL]
    ).abs()

    non_fail_df["iou_deviation_signed"] = (
        non_fail_df[PERT_IOU_COL] - non_fail_df[ORIG_IOU_COL]
    )
...
def compute_threshold_summary(df_non_fail: pd.DataFrame):
    deviations = df_non_fail["iou_deviation_abs"].to_numpy(dtype=float)
    n = len(deviations)
...
def print_model_results(model_name: str, df_all: pd.DataFrame, df_non_fail: pd.DataFrame, summary: dict):
    ...
    print(f"Mean abs IoU deviation           : {summary['mean_abs_deviation']:.6f}")
    print(f"Median abs IoU deviation         : {summary['median_abs_deviation']:.6f}")
    print(f"Std abs IoU deviation            : {summary['std_abs_deviation']:.6f}")
    print(f"Min abs IoU deviation            : {summary['min_abs_deviation']:.6f}")
    print(f"Max abs IoU deviation            : {summary['max_abs_deviation']:.6f}")

    print("\nFixed-threshold localization stability categories:")
    print(
...
def main():
    ...
    for model_name in MODEL_ORDER:
        ...
        df_export["stability_category"] = df_export["iou_deviation_abs"].apply(categorize_deviation)
        df_export["is_minor"] = (df_export["iou_deviation_abs"] <= MINOR_THR).astype(int)
        df_export["is_small_change"] = (
            (df_export["iou_deviation_abs"] > MINOR_THR) &
            (df_export["iou_deviation_abs"] <= SMALL_CHANGE_THR)
        ).astype(int)
        df_export["is_moderate_change"] = (
            (df_export["iou_deviation_abs"] > SMALL_CHANGE_THR) &
            (df_export["iou_deviation_abs"] <= MODERATE_CHANGE_THR)
        ).astype(int)
        df_export["is_large_change_violation"] = (
            df_export["iou_deviation_abs"] > VIOLATION_THR
        ).astype(int)
        ...
        all_summary_rows.append({
            ...
            "mean_abs_iou_deviation": summary["mean_abs_deviation"],
            "median_abs_iou_deviation": summary["median_abs_deviation"],
            "std_abs_iou_deviation": summary["std_abs_deviation"],
            "min_abs_iou_deviation": summary["min_abs_deviation"],
            "max_abs_iou_deviation": summary["max_abs_deviation"],
